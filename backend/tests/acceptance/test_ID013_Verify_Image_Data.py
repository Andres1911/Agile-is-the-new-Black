...
def given_user_is_member(db, username, household_name):
    ...
    membership = (
        ...
        .filter(HouseholdMember.user_id == user.id, HouseholdMember.household_id == household.id)
        .first()
    )
...
def given_user_logged_in(client, username, ocr_context):
    ...
    """
    ocr_context["auth_headers"] = get_auth_header(
        client, username=username, password="Password123!"
    )
...
@given(
    parsers.parse(
        'the system has processed the image and extracted amount "{amount}" from the receipt'
    ),

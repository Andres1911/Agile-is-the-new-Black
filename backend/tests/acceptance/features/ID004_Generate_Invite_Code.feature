Feature: Generate Invite Code
    ...
    Scenario Outline: ID004 Household Admin Generates a Random Invite Code for Household    (Normal Flow)
    ...
    Scenario Outline: ID004 Household Admin Manually Enters an Invite Code for Household    (Alternative Flow)
        Given a user with username "<UserName>" already exists in the system
        And the user "<UserName>" is an Admin of the household "<HouseholdName>"
        And the user "<UserName>" is logged in
    ...
    Scenario Outline: ID004 Non-Admin of Household attempts to generate household invite code    (Error Flow)
        Given a user with username "<UserName>" already exists in the system
        And the user "<UserName>" is logged in

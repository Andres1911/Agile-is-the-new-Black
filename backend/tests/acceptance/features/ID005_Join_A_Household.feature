Feature: Join A Household
 ...
 # --- Normal Flow ---
  ...
  Scenario Outline: ID005 User joins a household with correct invite code    (Normal Flow)
 ...
 # --- Error Flows ---
  ...
  Scenario Outline: ID005 User attempts to join a household with an incorrect invite code    (Error Flow)
    Given the user "<UserName>" does not currently belong to any household
    And a household named "<HouseholdName>" exists in the system
  ...
  Scenario Outline: ID005 User attempts to join a non-existent household    (Error Flow)
    Given the user "<UserName>" does not currently belong to any household
  ...
  Scenario Outline: ID005 Already in household user attempts to join another household    (Error Flow)

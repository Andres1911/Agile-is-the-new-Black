Feature: Accept or decline a requested expense
  ...
  I want to accept/decline my pending expense shares
  ...
  Background:
    ...
    And that expense has the following expense shares
      ...
      | Bob         | 20.00    | 0.00           | PENDING     |
      | Cara        | 40.00    | 0.00           | PENDING     |
  ...
  Scenario: ID015 Household member rejects a pending expense share    (Normal Flow)
    ...
    Then the expense shares for "Grocery run" should be as follows
      | participant | shareCAD | outstandingCAD | vote_status |
      | Bob         | 20.00    | 0.00           | PENDING     |
      | Cara        | 40.00    | 0.00           | REJECTED    |
  ...
  Scenario: ID015 All household members accept their pending expense shares    (Normal Flow)
    ...
    Then the status for the "Grocery run" expense should be "ACCEPTED"
    And the expense shares for "Grocery run" should be as follows:
      | participant | shareCAD | outstandingCAD | vote_status |
      | Bob         | 20.00    | 0.00           | ACCEPTED    |
      | Cara        | 40.00    | 0.00           | ACCEPTED    |

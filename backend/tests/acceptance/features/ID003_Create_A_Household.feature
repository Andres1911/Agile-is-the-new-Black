Feature: Create a household
  ...
  Scenario Outline: ID003 Create a New Household    (Normal Flow)
  ...
  Scenario Outline: ID003 Create a New Household with Address    (Alternative Flow)
    Given the user "<UserName>" hasn't been assigned a household
    And a household named "<Name>" does not exist
  ...
  Scenario Outline: ID003 Attempt to Create a Household with an existing Name    (Error Flow)
  ...
  Scenario Outline: ID003 Attempt to Create a New Household While Already Lived in a household    (Error Flow)

...
@when(
    parsers.parse('"{username}" creates and splits the expense among the following members'),
    target_fixture="context",
)
    ...
    for row in table_rows:
        ...
        if payer_name in name_to_id:
            context["manual_shares"].append(
                {"user_id": name_to_id[payer_name], "amount": float(row["shareCAD"])}
            )

    include_creator = any(
        share["user_id"] == name_to_id[username] for share in context["manual_shares"]
    )
...
@when(
    parsers.parse(
        '"{username}" creates and splits the expense equally with include_self="{is_inclusive}"'
    ),
    target_fixture="context",
)
...
@then(
    parsers.parse(
        'the system records an expense for "{username}" in "{h_name}" with amount "{amount}", description "{desc}", category "{cat}", and status "{status}"'
    )
)
...
def then_verify_expense_shares_all_attributes(db, context, datatable):
    ...
    assert expense is not None, (
        "Expense object not found in context. Check if the previous creation step succeeded."
    )
    ...
    actual_shares = db.query(ExpenseShare).filter(ExpenseShare.expense_id == expense.id).all()
    ...
    for expected in expected_shares:
        ...
        assert user is not None, (
            f"Test data error: User '{expected['user']}' does not exist in the database."
        )
        ...
        assert actual.expense_id == expense.id, (
            f"Linkage error: expense_id mismatch for {user.username}."
        )

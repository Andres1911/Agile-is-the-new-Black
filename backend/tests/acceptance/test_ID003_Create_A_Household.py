...
def step_user_homeless(username):
    ...
    active_membership = (
        ...
        .filter(HouseholdMember.user_id == user.id, HouseholdMember.left_at is None)
        .first()
    )

    assert active_membership is None, (
        f"Pre-condition failed: User {username} is already linked to a household!"
    )
...
def when_request_add_household(client, context, name):
    ...
    context["response"] = client.post(
        "/api/v1/households/", json=payload, headers=context["headers"]
    )
    return context


@when(
    parsers.parse('requesting the addition of household "{name}" with address "{address}"'),
    target_fixture="context",
)
...
@then(
    parsers.parse(
        'a household named "{name}" with address "{address}" should be created successfully'
    )
)
...
@then(
    parsers.parse('a binding record should link User "{username}" to Household "{name}"'),
    target_fixture="context",
)
...
@then(
    parsers.parse(
        'the old binding record should still link User "{username}" to Household "{current_home}"'
    )
)
...
@then(
    parsers.parse(
        'no binding record should exist between User "{username}" and Household "{new_home}"'
    )
)

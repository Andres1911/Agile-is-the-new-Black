...
@given(
    parsers.parse(
        'another household named "{target_house}" exists with the invite code "{valid_code}"'
    )
)
...
@when(
    parsers.parse(
        'the user requests to join household "{household_name}" with invite code "{invite_code}"'
    )
)
def when_request_join(client, context, household_name, invite_code):
    ...
    resp = client.post("/api/v1/households/join", json=payload, headers=context["auth_headers"])
...
@then(
    parsers.parse('a binding record should link User "{username}" to Household "{household_name}"')
)
...
@then(
    parsers.parse(
        'no binding record should exist between User "{username}" and Household "{target_house}"'
    )
)
